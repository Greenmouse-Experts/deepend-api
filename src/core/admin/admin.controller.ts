...
export class AdminController {
  ...
  async createFoodCategory(@Body() body: CreateFoodCategoryDto) {
    return await this.adminService.createFoodCategory(body.name, body.description,body.icon);
  }
  ...
  async updateFoodAddonCategory(@Param('id',ParseIntPipe) id: number, @Body() body: UpdateFoodCategoryDto) {
    return await this.adminService.updateFoodAddonCategory(id, body.name, body.description);
  }
  ...
  async updateFoodAddonItem(@Param('id',ParseIntPipe) id: number, @Body() body: UpdateFoodAddonItemDto) {
    return await this.adminService.updateFoodAddonItem(id,{name: body.name, price: body.price,categoryId: body.categoryId});
  }
  ...
  @ApiOperation({ summary: 'Get all food addon items by category ID with pagination' })
  async getFoodAddonItemsByCategoryId(
    @Param('categoryId',ParseIntPipe) categoryId: number,
    @Query(new QueryJoiValidationPipe(PaginationQuerySchema))
    { page, limit }: PaginationQueryDto
  ) {
    return await this.adminService.getAllFoodAddonItemsByCategory(categoryId, page, limit);
  }
  ...
  async addFoodAddons(
    ...
    return await this.adminService.addFoodAddons({foodId, addons: body.addons });
  }

  @Delete('foods/:foodId/addons/categories'
  )
  @ApiOperation({ summary: 'Remove an associated food addon categories from a food item' })
  ...
  async removeFoodAddonItem(
    ...
    return await this.adminService.removeFoodAddonCategory(foodId, body.addonCategoryIds );
  }

 @Delete('foods/:foodId/addons/items')
  @ApiOperation({ summary: 'Remove an associated food addon items from a food item' })
  @UsePipes(new  JoiValidationPipe(RemoveFoodAddonItemsSchema))
  async removeFoodAddonItems(
    @Param('foodId') foodId: string,
    @Body() body: RemoveFoodAddonItemsDto
  ) {
    return await this.adminService.removeFoodAddonItems(foodId, body.addonItemIds );
  }
