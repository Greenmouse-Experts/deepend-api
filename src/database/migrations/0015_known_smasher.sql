CREATE TABLE `studios_availability` (
	...
	CONSTRAINT `uk_studio_availability_unique` UNIQUE(`studio_id`,`day_of_week`,`start_time`,`end_time`),
	CONSTRAINT `chk_studio_availability_time` CHECK(`studios_availability`.`start_time` < `studios_availability`.`end_time`),
	CONSTRAINT `chk_studio_availability_status` CHECK(`studios_availability`.`status` IN ('available', 'booked', 'unavailable'))
);
...
ALTER TABLE `studios_availability` ADD CONSTRAINT `fk_studio_availability_studio_id` FOREIGN KEY (`studio_id`) REFERENCES `studios`(`id`) ON DELETE cascade ON UPDATE no action;